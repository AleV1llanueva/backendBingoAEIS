...
# 1. Cambiamos URLField por ImageField para almacenamiento local
    qr_code = models.ImageField(upload_to='qr_codes/', blank=True, null=True)
    correo_enviado = models.BooleanField(default=False, blank=True)

    def save(self, *args, **kwargs):
        # 2. Si no tiene QR, lo generamos localmente en memoria y lo asignamos al campo
        if not self.qr_code:
            buffer = BytesIO()
            qr = qrcode.make(str(self.id))
            qr.save(buffer, format="PNG")
            ...
            # Creamos un nombre de archivo único usando su teléfono
            filename = f"{self.telefono}_{self.id}.png"
            
            # Guardamos el archivo binario directamente en el ImageField de Django
